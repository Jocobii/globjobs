...
import { FileDropZone as File } from '../typings/files';
import { DataContext, DataProvider } from '../contexts/AppContext';
...
function MaterialDropZone({
  ...
  const onDrop = useCallback(
    (receivedFiles: File[]) => {
      const newFiles = receivedFiles.map((file) => {
        const newFile = file;
        newFile.id = file.name + Math.random();
        return newFile;
      });
      filesSetter((prev: File[]) => [...prev, ...newFiles]);
    },
    [filesSetter],
  );
  ...
  const uploadedFiles = files.map(({ name, id }: File) => (
    <ListItem
      key={id}
      secondaryAction={(
