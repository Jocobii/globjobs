...
export default function Table({
  ...
  const color: string = theme.palette.mode === 'light' ? '#000' : '#fff';
  ...
  const columns: GridColumns<GridValueGetterParams<Rows>> = [
    ...
    },
    {
      ...
      field: 'completed',
      renderCell: (
        params: GridRenderCellParams<GridValueGetterParams<boolean>, GridValidRowModel>,
      ) => {
        const { row } = params;
        const { completed: value, isCanceled } = row;
        if (isCanceled) return <Chip color="error" label="Canceled" />;
        return <Chip color={value ? 'success' : 'warning'} label={value ? 'Completed' : 'In Progress'} />;
      },
      valueGetter: (
        params: GridRenderCellParams<GridValueGetterParams<boolean>, GridValidRowModel>,
      ) => {
